...
export default function ContractInteraction() {
  ...
  const updateContractInfo = async (contract) => {
    try {
      ...
      setTVL(ethers.formatEther(tvl))

      const lendingRate = await contract.LENDING_RATE()
      setLendingRate(ethers.formatUnits(lendingRate, 2))

      const borrowingRate = await contract.BORROWING_RATE()
      setBorrowingRate(ethers.formatUnits(borrowingRate, 2))
    } catch (error) {
  ...
  const handleTransaction = async (
    ...
    if (contract) {
      try {
        ...
        await tx.wait()
        console.log(`${errorMessage} successful`)
        await updateContractInfo(contract)
      } catch (error) {
        console.error(`Error ${errorMessage.toLowerCase()}:`, error)
      }
    }
  }

  const handleDeposit = () => handleTransaction(
    () => contract.deposit({ value: ethers.parseEther(amount || '0') }),
    'Deposit'
  )

  const handleWithdraw = () => handleTransaction(
    () => contract.withdraw(ethers.parseEther(amount || '0')),
    'Withdrawal'
  )

  const handleBorrow = () => handleTransaction(
    () => contract.borrow(ethers.parseEther(amount || '0')),
    'Borrow'
  )

  const handleRepay = () => handleTransaction(
    () => contract.repay({ value: ethers.parseEther(amount || '0') }),
    'Repayment'
  )

  const handleLend = () => handleTransaction(
    () => contract.lend(ethers.parseEther(amount || '0')),
    'Lending'
  )

  const handleClaimRewards = () => handleTransaction(
    () => contract.claimLendingRewards(),
    'Claiming rewards'
  )
  ...
  return (
    <Card className="w-[350px]">
      <CardHeader>
        <CardTitle>SecureFinance Interaction</CardTitle>
        <CardDescription>Interact with the SecureFinance contract</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid w-full items-center gap-4">
          ...
          <div>
            ...
            <p>Total Value Locked: {tvl} ETH</p>
            <p>Lending Rate: {lendingRate}%</p>
            <p>Borrowing Rate: {borrowingRate}%</p>
          </div>
        </div>
      </CardContent>
